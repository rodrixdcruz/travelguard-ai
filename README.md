...
[![Coverage](https://raw.githubusercontent.com/rodrixdcruz/travelguard-ai/main/.github/coverage-badge.svg)](https://github.com/rodrixdcruz/travelguard-ai/actions/workflows/ci.yml)
...
the fallback paths. The suite currently reports 144 passing tests in CI, with
app-package coverage measured by `pytest-cov` on every run — the percentage in
the coverage badge above is generated by CI from the measured number and
committed back to this repo, so it can never drift from reality (no hardcoded
percentages, no third-party coverage service).
